...
# ? Any function below that says "read __" is a function that parses a specific part of the html
# ? and returns an according database object into a list
...
def main():
    ...
    for i in range(9):
        ...
        session = HTMLSession()
        r = session.get('https://classes.usc.edu/term/' + str(semester) + '/catalogue/ge')
        r.html.render()
        generalEducation = BeautifulSoup(r.content)
        generalEducation = generalEducation.find('app-root')
        print(generalEducation)
        generalEducation = generalEducation.find_all('div')
        print(generalEducation)
        ...
        r = urlSession.get('https://classes.usc.edu/term/' + str(semester) + '/catalogue/school')
        schools = BeautifulSoup(r.content, 'lxml')
        schools = schools.find('mat-accordion', class_='school-accordion')
        schools.extract()


        # get the schools and departments
        departments = departments.find_all('li') # ! ?
        readSchoolsDepartments(schools, schoolList, departmentList)
